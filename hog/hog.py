...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    point, roll_one=0, False
    while num_rolls>0:
    	dice_result=dice()
    	if dice_result==1:
    		roll_one=True
    	point=point+dice_result
    	num_rolls-=1
    if roll_one:
    	return 1
    return point
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    a, average_roll=9, make_averaged(roll_dice, num_samples)
    max_score, max_roll=average_roll(10, dice), 10
    while a>0:
    	if average_roll(a, dice)> max_score:
    		max_score=average_roll(a, dice)
    		max_roll=a
    	a-=1
    return max_roll
    # END PROBLEM 9
