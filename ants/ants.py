...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        n, mi, ma=self.place, self.min_range, self.max_range
        while mi:
            n, mi=n.entrance, mi-1
        while not n==hive and ma+1:
            bee=random_or_none(n.bees)
            if bee:
                return bee
            n, ma=n.entrance, ma-1
        return None
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            n=self.place
            while n.exit is not None:
                n=n.exit
                if n.ant is not None:
                    if hasattr(n.ant, 'damage') and n.ant not in self.doubled:
                        n.ant.damage*=2
                        self.doubled.append(n.ant)
                    if n.ant.container and n.ant.ant is not None and hasattr(n.ant.ant, 'damage') and n.ant.ant not in self.doubled:
                        n.ant.ant.damage*=2
                        self.doubled.append(n.ant.ant)
        else:
